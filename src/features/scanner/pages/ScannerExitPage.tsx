...
import { QrCode, RefreshCcw, CheckCircle2, AlertTriangle, User, ScanLine, Upload } from 'lucide-react'
...
export default function ScannerExitPage() {
  ...
  useEffect(() => {
    const scanner = new Html5Qrcode('qr-reader-exit')
  ...
  const handleScan = async (decodedText: string) => {
    ...
    try {
      ...
      if (vendor.status === 'OUTSIDE') {
        ...
        setErrorMsg('Vendor is already OUTSIDE. Cannot allow duplicate exit.')
        setScannerState('error')
        return
      }
  ...
  const handleAllowExit = async () => {
    ...
    try {
      const res = await scanService.processExit(scannedVendor.vendor_id, user?.email || 'Coordinator')
      if (res.success) {
        scanService.playSuccessBeep()
        success('Exit Allowed', `${scannedVendor.name} has been marked as OUTSIDE.`)
        resetScanner()
      } else {
        throw new Error(res.error || 'Failed to process exit.')
      }
    } catch (err: any) {
      showError('Exit Failed', err.message)
      scanService.playErrorBeep()
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <PageHeader 
        ...
        subtitle="Scan vendor QR codes as they leave the premises." 
      />
      ...
      <div className="bg-simmam-elevated border border-simmam-gold-border rounded-simmam-xl p-6 overflow-hidden">
        ...
        <div className={`relative ${scannerState === 'scanning' ? 'block' : 'hidden'}`}>
          <div id="qr-reader-exit" className="w-full rounded-xl overflow-hidden border-2 border-simmam-gold border-dashed" />
        ...
        {scannerState === 'error' && (
          <div className="text-center py-12">
            ...
            <h2 className="text-xl font-display font-bold text-simmam-text-primary mb-2">Exit Denied</h2>
          ...
          <AnimatePresence>
            ...
            >
              ...
              <div className="flex gap-4">
                ...
                <Button 
                  variant="outline" 
                  className="flex-1 border-simmam-gold text-simmam-gold hover:bg-simmam-gold hover:text-simmam-bg" 
